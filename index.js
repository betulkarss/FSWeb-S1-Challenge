...
function profilListesi(fenomen) {
  const result = [... fenomen];
  for ( let i =0; i<fenomen.length;i++){
    result[i]= fenomen[i].profile;
  }
  return result;
}
...
function fenomenSil(fenomenler,sayi) {
  const result2 = [...fenomenler];
     result2.splice(sayi,1);
     return result2;
}
...
function fenomenEkle(fenomenler,sırano,profilİsmi,takipciSayisi,gönderimSayisi,platformAdi) {
  const result3 = [...fenomenler];
  const yeniNesne = {
    "number": sırano,
    "profile": profilİsmi,
...
function enFenomenler(fenomenler) {
  const unluFenomenler =[];
  for(let i = 0 ; i<fenomenler.length;i++){
    if(fenomenler[i].followers > 100000000 ){
      unluFenomenler.push( fenomenler[i].profile);
    }
   
    }
    
    return unluFenomenler;
  }
...
function platformaGoreCokGonderiYapanFenomen(fenomenler,platform1){
      ...
      for (let i =0; i<fenomenler.length;i++){
        ...
        if(fenomenler[i].platform == platform1){
          gönderiler.push(fenomenler[i].posts)
          
        }
          
      }
      //const max = Math.max(...gönderiler)
      for(let i =0; i<fenomenler.length;i++){
        if(fenomenler[i].posts ==  Math.max(...gönderiler)){
          if(fenomenler[i].posts== "NA"){
            continue;
